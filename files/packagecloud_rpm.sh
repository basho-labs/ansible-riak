...
yum_repo_path=/etc/yum.repos.d/basho_riak.repo
yum_repo_config_url="https://packagecloud.io/install/repositories/basho/riak/config_file.repo?os=${os}&dist=${major_version}&name=${host}&source=script"
...
yum install -y pygpgme --disablerepo='basho_riak'
...
if [ "$?" != "0" ]; then
  ...
  sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/basho_riak.repo
fi

echo "Installing yum-utils..."
yum install -y yum-utils --disablerepo='basho_riak'
...
echo "Generating yum cache for basho_riak..."
yum -q makecache -y --disablerepo='*' --enablerepo='basho_riak'
